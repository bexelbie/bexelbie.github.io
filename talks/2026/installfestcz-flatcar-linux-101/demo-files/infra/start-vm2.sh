#!/bin/bash
# Boot Flatcar VM 2 (with Tailscale sysext) from the old version image.
...
# SSH: ssh core@192.168.122.102  (static IP)
...
IGN="${PROJECT_DIR}/vm2-sysext.ign"
WORK_IMG="${IMG_DIR}/vm2-work.qcow2"
...
if [ ! -f "$WORK_IMG" ]; then
  echo "==> Creating working copy for VM2 (qcow2, ~490M)"
  cp "${IMG_DIR}/flatcar_production_qemu_image.img" "$WORK_IMG"
fi

echo "==> Booting VM2 (Flatcar 4515.0.0 + Tailscale sysext)"
echo "    Network: virbr0 bridge (192.168.122.0/24)"
echo "    SSH: ssh core@192.168.122.102  (static IP)"
echo "    Stop: Ctrl-A X"
echo ""
