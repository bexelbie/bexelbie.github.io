#!/bin/bash
# ABOUTME: Boots the Demo 2 VM (sysext demo) from its pre-staged work image.
...
WORK_IMG="${SCRIPT_DIR}/assets/demo2-work.qcow2"
IGN="${SCRIPT_DIR}/demo2.ign"
...
echo "==> Booting Demo 2 VM (sysext demo)"
...
qemu-system-x86_64 \
  ...
  -device virtio-net-pci,netdev=net0,mac=52:54:00:fc:04:01,romfile="" \
  -boot order=c \
  -object rng-random,filename=/dev/urandom,id=rng0 -device virtio-rng-pci,rng=rng0 \
  </dev/null >/dev/null 2>&1 &

echo "    PID: $!"
echo "    SSH: ./go.sh  (or ssh core@192.168.122.104)"
echo "    Stop: ./stop.sh"
